...
lazy_static! {
    static ref REGEX_CACHE:Mutex< HashMap<&'static str, Regex>> = {
        let  mut m = HashMap::new();
        Mutex::new(m)
    };
}
...
fn match_single(
	...
	let mut map = REGEX_CACHE.lock().unwrap();
	let regex = map.entry(&command_regex).or_insert_with(||Regex::new(command_regex).unwrap());
...
fn match_multiple(
	...
	let command_re = map.entry(&command_regex).or_insert_with(||Regex::new(&command_regex).unwrap());
	let arg_re = arg_map.entry(&arg_regex).or_insert_with(||Regex::new(&arg_regex).unwrap());
	let mut result = vec![];
